...
export default function Overdue({ client, invoices }: OverdueProps) {
  return (
    <>
      <Head title={`Overdue · ${client.name}`} />

      <AppLayout title={`Overdue · ${client.name}`}>
        <div className="space-y-6">
          <section className="mb-8 overflow-hidden rounded-[2rem] border border-white/70 bg-white/85 p-8 shadow-[0_30px_80px_rgba(33,32,28,0.12)] backdrop-blur">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.35em] text-stone-500">Overdue report</p>
                <h1 className="mt-3 text-4xl font-semibold tracking-tight text-stone-950 md:text-5xl">
                  {client.name}
                </h1>
                <p className="mt-4 max-w-2xl text-base leading-7 text-stone-600">
                  Focus only on invoices that need follow-up so you can clear aging balances faster.
                </p>
              </div>
            ...
            {invoices.length === 0 ? (
              <div className="px-8 py-20 text-center">
                ...
                <p className="mx-auto mt-3 max-w-xl text-stone-600">
                  This client has no overdue balance at the moment.
                </p>
              </div>
            ) : (
